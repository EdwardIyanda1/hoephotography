  ...
  return {
    async write(path, source, expected, max) {
      ...
      try {
        await pipeline(
          ...
          new Transform({
            transform(chunk, encoding, callback) {
              bytes += chunk.length;
              callback(
                bytes > max || bytes > expected
                  ? fail(413, "File exceeds upload size")
                  : null,
                chunk,
              );
            },
        ...
        await link(temp, target); // Atomic create, never overwrite an existing file.
      } catch (error) {
        if (error.code === "EEXIST") throw fail(409, "File already uploaded");
        throw error;
      } finally {
    ...
    async info(path) {
      return { size: (await stat(absolute(path))).size };
    },
    async remove(path) {
    ...
    async serve(req, res, media, download = false) {
      ...
      await new Promise((resolve, reject) =>
        res.sendFile(
          path,
          { cacheControl: false, acceptRanges: true },
...
export function createMailer(
  ...
  return async (to, subject, text, key) => {
    const password = (settings.GMAIL_APP_PASSWORD || "").replace(/\s/g, "");
    ...
    transport ||= makeTransport({
      ...
      port: 465,
      secure: true,
      auth: { user: settings.GMAIL_USER, pass: password },
      connectionTimeout: 15000,
      socketTimeout: 30000,
    });
    return transport.sendMail({
      from: { name: "Hoe Studio", address: settings.GMAIL_USER },
      to,
      subject,
      text,
      ...(key
        ? {
            messageId: `<${createHash("sha256").update(key).digest("hex")}@hoe-studio.local>`,
          }
        : {}),
    });
  };
